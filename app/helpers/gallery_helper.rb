...
module GalleryHelper
  ...
  def galleria_link(picture)
    ...
    thumbnail_url = image.create_thumbnail(:width => 200, :height => 128, :crop => true).url.gsub(/\/(\d+-\d+!)\//, '/\1n/')
    image_url = image.create_thumbnail(:width => 1050, :height => 700, :crop => true).url.gsub(/\/(\d+-\d+!)\//, '/\1n/')
